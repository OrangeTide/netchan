...
flush(int fd, struct netchan_conn *c)
{
    ...
    for (;;) {
        ...
        n = netchan_send_next(c, buf, sizeof(buf), &dst);
        if (n == 0) break;
...
drain(int fd, struct netchan_conn *c)
{
    ...
    for (;;) {
        ...
        n = recvfrom(fd, pkt, sizeof(pkt), MSG_DONTWAIT,
...
read_chan_msg(struct netchan_conn *c, const char *want, size_t wlen)
{
    ...
    if (!ch) return 0;
    rd = netchan_chan_read(ch, buf, sizeof(buf));
...
{
    struct sockaddr_in caddr, saddr, caddr2;
    struct netchan_conn *cl, *sv;
    ...
    ch = netchan_chan_open(cl, NETCHAN_RELIABLE, NETCHAN_DIR_SEND, "state");
    CHECK(ch != NULL, "chan_open failed");
    pump_both(cfd, cl, sfd, sv);

    m1 = "over a real socket";
    ...
    m2 = "after moving to a new address";
